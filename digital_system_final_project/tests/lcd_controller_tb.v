...
module lcd_controller_tb;
    ...
    reg  [7:0] captured_data [0:639];
    reg        captured_rs   [0:639];
    ...
    always @(negedge LCD_EN) begin
        if (reset_n && (write_count < 640)) begin
    ...
    initial begin
        ...
        // Smart extension keeps the signal label but flashes two dashes in
        // the countdown field. Both visible and blank halves are framed.
        traffic_state     = ST_EW_GREEN;
        remaining_seconds = 16'd0;
        traffic_extended  = 1'b1;
        blink_visible     = 1'b1;
        wait_for_writes(517);
        check_write(496, 1'b1, "-");
        check_write(497, 1'b1, "-");

        blink_visible = 1'b0;
        wait_for_writes(585);
        check_write(564, 1'b1, " ");
        check_write(565, 1'b1, " ");
