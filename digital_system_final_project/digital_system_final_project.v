...
module digital_system_final_project #(
    ...
    // Synchronize asynchronous sensor and mode switches before they control
    ...
    // A full blink cycle is one second: 0.5 s visible and 0.5 s blank.
    always @(posedge CLOCK_50 or negedge reset_n) begin
        if (!reset_n) begin
            blink_count   <= 32'd0;
            blink_visible <= 1'b1;
    ...
    // SW[2:0] select smart sensing and vehicle demand, SW[3] selects night
    // mode, SW[4] simulates a fault, and SW[17] requests system settings.
